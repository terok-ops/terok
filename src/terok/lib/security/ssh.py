...
from ..core.projects import ProjectConfig, effective_ssh_key_name, load_project
from ..util.fs import ensure_dir_writable
from ..util.template_utils import render_template


class SSHManager:
    """Service class that manages SSH keypair generation and config for a project."""
    ...
    def init(
        ...
        /home/dev/.ssh and creates an SSH keypair plus a minimal config file
        if missing.
...
def init_project_ssh(
    ...
    This is a backward-compatible wrapper around :meth:`SSHManager.init`.
    It loads the project by *project_id*, creates an :class:`SSHManager`, and
    delegates to :meth:`SSHManager.init`.

    Returns a dict with keys: dir, private_key, public_key, config_path, key_name.
    """
    project = load_project(project_id)
    manager = SSHManager(project)
    return manager.init(key_type=key_type, key_name=key_name, force=force)
