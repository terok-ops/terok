...
separate key per project (`terok project ssh-init`); keys remain on the host and
are served to containers via the vault's SSH signer.
...
Each project has its own SSH key, generated by `terok project ssh-init`.
Keys are served to containers via the vault SSH signer.
