...
`terok sickbay` reports profile load + mode in a `Hardening
(AppArmor)` row alongside the SELinux equivalents. On a non-
AppArmor host the row reads `not applicable`.
