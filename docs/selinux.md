...
[`apparmor.md`](apparmor.md) — same `terok hardening install`
...
SELinux policy   WARN (policy NOT installed)
                 install: terok hardening install
```
...
`terok sickbay` reports the optional layer in a separate
`Hardening (SELinux)` row from `SELinux policy` (the connectto
rule), so you can see at a glance which layer needs attention.

## Removing

```bash
terok hardening remove
```
